...
In the 20×20 grid below, four numbers along a diagonal line have been marked (*nn*).
...
function largestGridProduct(arr) {
  let product = 0;
  let max = 0;
  // This 'for' could be optimized, but my goal is to solve the challenges
  for (let row = 0; row < arr.length; row++) {
    ...
    // calculate left
    for (let col = arr[0].length - 1; col >= 3; col--) {
      product = arr[row][col] * arr[row][col - 1] * arr[row][col - 2] * arr[row][col - 3];
      if (product > max)
        max = product;
    }
